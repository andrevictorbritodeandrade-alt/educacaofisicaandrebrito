...
const CATEGORIAS = [
  { id: '1º ANO DO ENSINO MÉDIO', icon: Users, color: 'bg-blue-100 text-blue-800 border-blue-400' },
  { id: 'EJANEM', icon: BookOpen, color: 'bg-purple-100 text-purple-800 border-purple-400' },
  { id: '8º ANO ENSINO FUNDAMENTAL', icon: Users, color: 'bg-green-100 text-green-800 border-green-400' },
  { id: 'AP', icon: FileText, color: 'bg-orange-100 text-orange-800 border-orange-400' }
];
...
export const BibliotecaEscolarView: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  const [showAltinhaPreLoaded, setShowAltinhaPreLoaded] = useState(false);
  ...
  if (view === 'home') {
    return (
      <div className="p-8 font-sans">
        <button onClick={onBack} className="text-slate-600 mb-4 flex items-center">
            <ArrowLeft className="w-5 h-5 mr-2" /> Voltar ao Painel
        </button>
        <h1 className="text-4xl font-extrabold text-slate-800 mb-8">Biblioteca Escolar</h1>
  ...
  if (view === 'categoria') {
    const arquivosDaCategoria = arquivos.filter(arq => arq.category === selectedCategory);
    const showPreLoadedButton = selectedCategory === '1º ANO DO ENSINO MÉDIO' || selectedCategory === 'EJANEM';
    const showAltinhaButton = selectedCategory === 'AP';

    return (
      <div className="p-8 font-sans">
        <button onClick={() => setView('home')} className="text-slate-600 mb-4 flex items-center"> <ArrowLeft className="w-5 h-5 mr-2" /> Voltar </button>
        <div className="flex justify-between items-center mb-10">
            <h1 className="text-4xl font-black text-slate-800">{selectedCategory}</h1>
        ...
        {showPreLoadedButton && (
            <button onClick={() => setShowPreLoaded(true)} className="w-full mb-8 bg-indigo-600 text-white p-6 rounded-2xl flex items-center text-2xl font-bold shadow-lg">
                <Presentation className="w-10 h-10 mr-6" /> Abrir Aula: Debate - Corpo e Mídia
            </button>
        )}
        
        {showAltinhaButton && (
            <button onClick={() => setShowAltinhaPreLoaded(true)} className="w-full mb-8 bg-emerald-600 text-white p-6 rounded-2xl flex items-center text-2xl font-bold shadow-lg">
                <Presentation className="w-10 h-10 mr-6" /> Abrir Aula: Altinha & Futevôlei
            </button>
        )}

        {showPreLoaded && <SlideViewer slideType="corpo-midia" onClose={() => setShowPreLoaded(false)} />}
        {showAltinhaPreLoaded && <SlideViewer slideType="altinha-futvolei" onClose={() => setShowAltinhaPreLoaded(false)} />}
